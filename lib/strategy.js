...
function OAuth2Strategy(options, verify) {
	...
	if (options.store) {
		...
		if (options.state) {
			this._stateStore = options.pkce
				? new PKCESessionStateStore({ key: this._key })
				: new SessionStateStore({ key: this._key });
		} else {
			if (options.pkce) {
	...
	console.log('[passport-oauth2] contructor', { options });
}
...
OAuth2Strategy.prototype.authenticate = function(req, options) {
	options = options || {};
	var self = this;
	console.log('[passport-oauth2] authenticate', { options });

	if (req.query && req.query.error) {
		if (req.query.error == 'access_denied') {
			console.log('[passport-oauth2] authenticate error :154', { query: req.query });
			return this.fail({ message: req.query.error_description });
		} else {
			console.log('[passport-oauth2] authenticate error :157', { query: req.query });
			return this.error(
				new AuthorizationError(req.query.error_description, req.query.error, req.query.error_uri)
			);
		}
	}
	...
	if (callbackURL) {
		console.log('[passport-oauth2] authenticate callbackURL :166', { callbackURL });
		var parsed = url.parse(callbackURL);
		if (!parsed.protocol) {
			// The callback URL is relative, resolve a fully qualified URL from the
			// URL of the originating request.
			console.log('[passport-oauth2] authenticate callbackURL :171', { callbackURL });
			callbackURL = url.resolve(utils.originalURL(req, { proxy: this._trustProxy }), callbackURL);
		}

		console.log('[passport-oauth2] authenticate callbackURL :175', { parsed });
	}
	...
	if (req.query && req.query.code) {
		console.log('[passport-oauth2] authenticate code :185', { query: req.query });
		function loaded(err, ok, state) {
			console.log('[passport-oauth2] loaded :187', { err, ok, state });
			if (err) {
				console.log('[passport-oauth2] loaded err :189', { err, ok, state });
				return self.error(err);
			}
			if (!ok) {
				console.log('[passport-oauth2] loaded !ok :193', { err, ok, state });
			...
			self._oauth2.getOAuthAccessToken(code, params, function(
				...
				console.log('[passport-oauth2] loaded getOAuthAccessToken :215', {
					err,
					accessToken,
					refreshToken,
					params
				...
				self._loadUserProfile(accessToken, function(err, profile) {
					console.log('[passport-oauth2] loaded _loadUserProfile :227', {
						err,
						profile
					});
					...
					function verified(err, user, info) {
						console.log('[passport-oauth2] loaded verified :237', {
							err,
							user,
							info
						});
					...
					try {
						...
						console.log('[passport-oauth2] loaded catch error :275', {
							ex
						});
		...
		if (this._pkceMethod) {
			...
			switch (this._pkceMethod) {
				...
				default:
					return this.error(
						new Error('Unsupported code verifier transformation method: ' + this._pkceMethod)
					);
			}
...
OAuth2Strategy.prototype._loadUserProfile = function(accessToken, done) {
	...
	if (typeof this._skipUserProfile == 'function' && this._skipUserProfile.length > 1) {
		...
		var skip =
			typeof this._skipUserProfile == 'function' ? this._skipUserProfile() : this._skipUserProfile;
		if (!skip) {
			return loadIt();
		}
		return skipIt();
